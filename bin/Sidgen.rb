...
# Function to create a SID file with correct padding
def create_sid_file(melody, filename, pattern_length = 16, play_address = 0)
  # Calculate the data size based on the length of the melody
  data_size = melody.size * 7

  # Calculate the padding size to make the total size a multiple of 0x7C
  padding_size = (0x7C - (data_size % 0x7C)) % 0x7C

  # Construct the SID file header with padding
  header = "#{MAGIC_NUMBER.ljust(4)}#{VERSION.to_s.rjust(4, '0')}"
  header += [data_size + 2 + padding_size].pack('v')  # Data size (plus 2 for play address)
  ...
  # Add the padding to the SID data
  sid_data.concat([0x00] * padding_size)
